...
def transgenerate(request):
    ...
    if transaction == '1':
        if report == '1':
            ...
            print "subledger per payee", transaction
...
def querySubledger(dto, dfrom, transaction, chartofaccount):
    ...
    # query = "SELECT a.id, a.document_type, a.document_id, a.document_num, a.document_date, a.subtype, a.particulars, a.chartofaccount_id, a.document_reftype, a.document_refnum, a.document_refdate, a.amount, "\
    ...
    #         "WHERE a.chartofaccount_id IN (SELECT id FROM chartofaccount WHERE main = '1' AND isdeleted=0 AND accounttype='P' AND prepaid_enable='Y' ORDER BY accountcode) "\
    #         "AND DATE(document_date) >= '"+str(dfrom)+"' AND DATE(document_date) <= '"+str(dto)+"' "+str(orderby)
    query = """
